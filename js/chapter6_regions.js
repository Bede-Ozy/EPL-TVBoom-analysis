/**
 * Improvement 4: Quick-Focus Regional Filters
 * 
 * Implements camera preset zooms to target coordinates on the map.
 */
...
Promise.all([
    ...
    // Process flows and country statistics
    const flows = [];
    const flowMap = {};
    const countryStats = {};
    ...
    // Draw countries using baseline static styling
    const countryFeatures = mapGroup.append("g")
        ...
        .on("mouseout", function() {
            ...
            if (activeRegion) {
                const activeList = regions[activeRegion] || [];
                countryFeatures.style("opacity", d => {
                    const name = d.properties.name;
                    if (name === "England") return 1.0;
                    return activeList.includes(name) ? 1.0 : 0.05;
                });
                
                d3.selectAll(".flow-line").style("opacity", d => {
                    const partner = d.to === "England" ? d.from : d.to;
                    const geoName = csvToGeoJsonName(partner);
                    return activeList.includes(geoName) ? 0.3 : 0;
                });

                d3.selectAll(".flow-animation-line").style("opacity", d => {
                    const partner = d.to === "England" ? d.from : d.to;
                    const geoName = csvToGeoJsonName(partner);
                    return activeList.includes(geoName) ? 0.7 : 0;
                });
            } else {
                countryFeatures.style("opacity", d => {
                    const name = d.properties.name;
                ...
                d3.selectAll(".flow-line").style("opacity", 0.3);
    ...
    // 3. Connect regional zoom control buttons to D3 matrix projection transitions
    const regions = {
        europe: ["Spain", "Italy", "Germany", "France", "Netherlands", "Portugal", "Russia", "Scotland", "Denmark", "Switzerland", "Ireland", "Slovenia", "Türkiye", "Latvia", "Bulgaria", "Slovakia", "Belgium", "Ukraine", "Norway", "Croatia", "Greece", "Sweden", "Serbia", "Czech Republic", "Wales", "Austria", "Poland", "Hungary", "Romania", "Northern Ireland", "Cyprus", "Finland"],
        samerica: ["Brazil", "Argentina", "Chile", "Uruguay", "Colombia", "Venezuela", "Paraguay", "Ecuador"],
        namerica: ["United States", "USA", "Mexico", "Costa Rica", "Honduras", "Trinidad and Tobago"],
        africa: ["Togo", "South Africa", "Ivory Coast", "Cote d'Ivoire", "Egypt", "Angola", "Nigeria"]
    };
    ...
    function filterByRegion(regionKey) {
        ...
        if (!regionKey) {
            countryFeatures.transition().duration(500)
                .style("opacity", d => {
                    const name = d.properties.name;
                    if (name === "England") return 1.0;
                    return countryStats[name] ? 1.0 : 0.15;
        ...
        const activeList = regions[regionKey] || [];
        ...
        countryFeatures.transition().duration(500)
            ...
            .style("pointer-events", d => {
                const name = d.properties.name;
                return (name === "England" || activeList.includes(name)) ? "auto" : "none";
            });
        ...
        d3.selectAll(".flow-line")
            .style("pointer-events", d => {
                const partner = d.to === "England" ? d.from : d.to;
                const geoName = csvToGeoJsonName(partner);
                return activeList.includes(geoName) ? "auto" : "none";
            })
            .transition().duration(500)
            .style("opacity", d => {
                const partner = d.to === "England" ? d.from : d.to;
    ...
    function zoomToCoords(lon, lat, scaleVal) {
        const point = projection([lon, lat]);
        const translate = [width / 2 - scaleVal * point[0], height / 2 - scaleVal * point[1]];
        svg.transition()
            .duration(1000)
            .call(zoom.transform, d3.zoomIdentity.translate(translate[0], translate[1]).scale(scaleVal));
    }
    ...
    d3.select("#focus-europe").on("click", () => {
        zoomToCoords(12, 50, 3.2);
        filterByRegion("europe");
    });
    d3.select("#focus-samerica").on("click", () => {
        zoomToCoords(-60, -18, 2.2);
        filterByRegion("samerica");
    });
    d3.select("#focus-namerica").on("click", () => {
        zoomToCoords(-98, 38, 1.8);
        filterByRegion("namerica");
    });
    d3.select("#focus-africa").on("click", () => {
        zoomToCoords(18, 2, 2.0);
        filterByRegion("africa");
    });
    d3.select("#focus-reset").on("click", () => {
        resetZoom();
        filterByRegion(null);
    });

    // Standard HUD zoom controls
