/**
 * Improvement 3: Interactive Timeline Slider
 * 
 * Filters transfer datasets dynamically year-by-year from 2002 to 2025.
 * Includes play/pause playback loop controls.
 */
...
Promise.all([
    ...
    // Set up D3 groups
    const countriesLayer = mapGroup.append("g").attr("class", "countries-layer");
    const flowsLayer = mapGroup.append("g").attr("class", "flows-layer");
    
    const strokeWidthScale = d3.scaleSqrt().range([1.2, 7]);
    ...
    // Draw static countries first
    const countryFeatures = countriesLayer.selectAll("path")
        .data(world.features)
        .enter()
        .append("path")
        .attr("d", path)
    ...
    // Timeline elements binding
    const playBtn = d3.select("#play-btn");
    const slider = d3.select("#time-slider");
    const yearInfo = d3.select("#year-info");
    const allBtn = d3.select("#all-btn");

    function updateTimelineMap() {
        // 1. Filter arrays
        const filterFn = activeYear === "all" 
            ? (d => d.season_start >= 2002) 
            : (d => +d.season_start === +activeYear);

        const filteredIn = transfersIn.filter(filterFn);
        const filteredOut = transfersOut.filter(filterFn);
        const filteredAll = allTransfers.filter(filterFn);
        ...
        strokeWidthScale.domain([0, maxFee]);

        // 3. Update Country styling classes (Original theme shading)
        countryFeatures
            .attr("class", d => {
            ...
            .on("end", function() {
                // Ensure events are bound/active
            });

        countryFeatures.on("mouseover", function(event, d) {
                const name = d.properties.name;
        ...
        // 4. Update curves and animation lines
        flowsLayer.selectAll("*").remove();
    ...
    // Connect control UI handlers
    slider.on("input", function() {
        activeYear = +this.value;
        yearInfo.html(`Year: <span class="year-label">${activeYear}</span>`);
        updateTimelineMap();
    });

    allBtn.on("click", () => {
        activeYear = "all";
        yearInfo.html('Year: <span class="year-label">All (2002-2025)</span>');
        slider.property("value", "2025");
    ...
    function togglePlay() {
        if (isPlaying) {
            ...
            if (activeYear === "all" || activeYear >= 2025) {
                activeYear = 2002;
            }

            slider.property("value", activeYear);
            yearInfo.html(`Year: <span class="year-label">${activeYear}</span>`);
            updateTimelineMap();
